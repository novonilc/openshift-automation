...
# 2017.10.17   Saul Alanis           Deployed to DEV CHG0184841
...
if [[ ! -z "$ID" ]]; then
  mv $ID $DATA_DIR/etcd-reshift-`date +%Y%m%d%M`.zip
fi

# initiate etcd dir backup
# https://docs.openshift.com/container-platform/3.5/admin_guide/backup_restore.html
if [[ $? -eq 0 ]]; then
  /usr/bin/ansible-playbook /usr/local/etc/pb_etcd_backup.yaml -v
    if [[ $? -eq 0 ]]; then
